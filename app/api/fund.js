 ...
 * @returns {Promise<Array>} - 历史净值数据数组，按日期升序排列
 */
export const fetchFundHistoryAll = async (code) => {
  if (typeof window === 'undefined') return [];

  // 东方财富历史净值API - 分页获取所有数据
  const allData = [];
  let page = 1;
  const perPage = 500; // 每页500条

  try {
    while (true) {
      // 不指定 sdate 和 edate，获取从最早到现在的所有数据
      const url = `https://fundf10.eastmoney.com/F10DataApi.aspx?type=lsjz&code=${code}&page=${page}&per=${perPage}`;
    ...
    console.error('Fetch history all error:', e);
    return [];
  }
};
